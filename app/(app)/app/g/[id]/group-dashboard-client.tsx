...
export function GroupDashboardClient({
	...
	const [volumeRange, setVolumeRange] = useState<"today" | "week" | "month">(
		"week"
	);
            ...
            `
					...
					if (updatedCheckins) {
						const formattedCheckins = updatedCheckins.map((checkin) => {
							const habit = Array.isArray(checkin.habit)
								? checkin.habit[0]
								: checkin.habit;
							const owner = Array.isArray(habit?.owner)
								? habit.owner[0]
								: habit?.owner;
	...
	const calculateVolume = (
		...
		return data.checkins.filter(
			(checkin) =>
				...
				isWithinInterval(new Date(checkin.createdAt), {
					start: startDate,
					end: endDate,
				})
		).length;
	...
	const sortedMembers = [...getMembersWithMetrics()].sort(
		(a: MemberWithVolume, b: MemberWithVolume) => {
			if (leaderboardView === "streaks") {
				...
				const getVolume = (member: MemberWithVolume) => {
					switch (volumeRange) {
						case "today":
							return member.todayVolume;
						case "week":
							return member.weekVolume;
						case "month":
							return member.monthVolume;
					}
				};
			...
			<div className="flex items-center gap-4">
				...
				<div>
					...
					<p className="text-muted-foreground">
						Led by {data.group.ownerUsername} • {data.members.length} member
							...
							<Card>
								<CardContent className="flex flex-col items-center justify-center py-8">
									...
									<p className="text-sm text-muted-foreground">
										Check-ins will appear here when group members log progress
									</p>
								</CardContent>
							</Card>
						) : (
											...
											<div>
												...
												{leaderboardView === "streaks" ? (
													...
													<div className="flex items-center gap-4 text-sm text-muted-foreground">
														<span>
															{volumeRange === "today" &&
																`${(member as MemberWithVolume).todayVolume} habits today`}
															{volumeRange === "week" &&
																`${(member as MemberWithVolume).weekVolume} habits this week`}
															{volumeRange === "month" &&
																`${(member as MemberWithVolume).monthVolume} habits this month`}
														</span>
													</div>
												)}
											</div>
					...
					{removeMessage && (
						<div
							className={`p-3 rounded-md text-sm ${
								removeMessage.includes("successfully") ||
								removeMessage.includes("removed")
							...
							<Card key={member.userId}>
								<CardContent className="pt-4">
									<div className="flex items-center gap-4">
										...
										{(data.group.userRole === "owner" ||
											data.group.userRole === "admin") && (
											<div className="flex items-center gap-2">
												{member.role !== "owner" && (
													<Button
														variant="outline"
